...
$data = mysqli_query($conn, "SELECT item.item_name, item_request.employee_id,item_request.quantity,employee.name,item_request.id,item_request.status,item_request.request_date
        FROM item_request
        LEFT JOIN employee ON item_request.employee_id = employee.id
          LEFT JOIN item ON item_request.item_id = item.item_id order by id desc"); 
    ...
    <table class="table table-borderless">
        ...
        if ($total != 0) {
            while ($result = mysqli_fetch_assoc($data)) {
                if($result['employee_id'] ==$_SESSION['eid'] )
                {
                    echo '
                    <tr>
